            ...
            <div class="max-w-2xl mx-auto">
                <div class="bg-white/70 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_60px_rgba(0,0,0,0.10)] border border-white/80 p-8 lg:p-10">
                    ...
                    <div id="lead-step-3" class="space-y-5 hidden animate-fade-in">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-[11px] font-bold text-navy-400 uppercase tracking-wider">Date souhaitée (optionnel)</label>
                            <div class="relative">
                                <i data-lucide="calendar-days" class="absolute left-3.5 top-1/2 -translate-y-1/2 text-navy-300 pointer-events-none" style="width:16px;height:16px;"></i>
                                <input type="date" id="lead-preferred-date" class="form-input pl-10 rounded-xl bg-navy-50/60 border-navy-100 focus:bg-white">
                            </div>
                        </div>

                        <div class="space-y-1.5">
                            <label class="text-[11px] font-bold text-navy-400 uppercase tracking-wider">Document (optionnel)</label>
                            <input type="file" id="lead-doc" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg"
                                   class="form-input rounded-xl bg-navy-50/60 border-navy-100 focus:bg-white file:mr-3 file:rounded-lg file:border-0 file:bg-brand-600 file:px-3 file:py-1.5 file:text-xs file:font-bold file:text-white hover:file:bg-brand-700">
                            <p class="text-[11px] text-navy-400">Formats: PDF, DOC/DOCX, XLS/XLSX, PNG, JPG (max 10MB).</p>
                        </div>
...
function goToLeadStep(step) {
    ...
    if (step === 4) {
        ...
        const preferredDate = document.getElementById('lead-preferred-date').value;
        const docInput = document.getElementById('lead-doc');
        const docName = docInput && docInput.files && docInput.files[0] ? docInput.files[0].name : '';
        ...
        document.getElementById('lead-summary').innerHTML = `
            ...
            ${preferredDate ? `
            <div class="flex items-center justify-between border-b border-navy-100 pb-3">
                <span class="text-sm font-bold text-navy-400">Date souhaitée</span>
                <span class="font-bold text-navy-900 bg-navy-50 px-3 py-1 rounded-lg">${preferredDate}</span>
            </div>` : ''}
            ${docName ? `
            <div class="flex items-center justify-between border-b border-navy-100 pb-3">
                <span class="text-sm font-bold text-navy-400">Document</span>
                <span class="font-bold text-navy-900 bg-navy-50 px-3 py-1 rounded-lg truncate max-w-[60%]">${docName}</span>
            </div>` : ''}
...
async function submitLead() {
    ...
    formData.append('preferred_date', document.getElementById('lead-preferred-date').value || '');
    formData.append('first_name', firstName);
    formData.append('last_name', lastName);
    formData.append('phone', document.getElementById('lead-phone').value.trim());
    formData.append('email', email);
    formData.append('zip_code', document.getElementById('lead-zip').value.trim());
    formData.append('city', document.getElementById('lead-city').value.trim());
    const docFile = document.getElementById('lead-doc').files[0];
    if (docFile) {
    ...
    try {
        ...
        if (trackLink && !Auth.isLoggedIn()) {
            trackLink.href = `/rappel/public/client/login.php?email=${encodeURIComponent(email)}`;
        } else if (trackLink) {
...
function resetLeadForm() {
    ...
    // Reset inputs
    ['lead-firstname','lead-lastname','lead-phone','lead-email','lead-zip','lead-city','lead-preferred-date'].forEach(id => {
        const el = document.getElementById(id);
        if (el) { el.value = ''; el.classList.remove('border-red-300','bg-red-50'); }
    });
    const leadDoc = document.getElementById('lead-doc');
    if (leadDoc) leadDoc.value = '';
